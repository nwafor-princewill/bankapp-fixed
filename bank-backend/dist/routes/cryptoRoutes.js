...
const CryptoTransaction_1 = __importDefault(require("../models/CryptoTransaction"));
const router = (0, express_1.Router)();
// --- GET ROUTES (for reading data) ---
// @route   GET /api/crypto/wallets
router.get('/wallets', auth_1.default, async (req, res) => {
    try {
        const user = await User_1.default.findById(req.user?.id).select('cryptoWallets');
        if (!user) {
...
router.get('/transactions', auth_1.default, async (req, res) => {
    try {
        const transactions = await CryptoTransaction_1.default.find({ userId: req.user?.id })
            .sort({ timestamp: -1 })
            .limit(50);
...
router.post('/wallets', auth_1.default, async (req, res) => {
    try {
...
router.post('/transactions', auth_1.default, async (req, res) => {
    try {
        ...
        const txExists = await CryptoTransaction_1.default.findOne({ txid });
        if (txExists) {
        ...
        const newTransaction = await CryptoTransaction_1.default.create({
            userId: req.user?.id,
            walletAddress,
            txid,
