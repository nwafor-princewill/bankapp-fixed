export default function BusinessGuidePage() {
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6 text-[#03305c]">Small Business Banking Guide</h1>
      <div className="space-y-6 text-gray-700 text-lg">
        <p>
          This comprehensive guide covers all aspects of Amalgamated Bank's small 
          business banking services, from account setup to advanced cash management.
        </p>
        
        <p>
          Learn about our business checking and savings options, merchant services, 
          payroll solutions, and commercial lending products.
        </p>
        
        <p>
          The guide includes tips for managing business finances, fraud prevention 
          best practices, and how to integrate your banking with accounting software.
        </p>
        
        <p className="mt-8 text-[#03305c] font-medium">
          Our business banking specialists are available to help at 
          amalgamateedbank@gmail.com.
        </p>
      </div>
    </div>
  );
}