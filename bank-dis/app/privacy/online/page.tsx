export default function OnlinePrivacyPage() {
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6 text-[#03305c]">Online Privacy Policy</h1>
      <div className="space-y-4 text-gray-700">
        <p>
          This policy explains how ZenaTrust handles information when you use our online services.
        </p>
        <p>
          We use industry-standard security measures to protect your online transactions.
        </p>
        <p>
          For questions about online privacy, email zenatrustbank@gmail.com.
        </p>
      </div>
    </div>
  );
}