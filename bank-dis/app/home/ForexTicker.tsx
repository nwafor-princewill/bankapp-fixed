...
const ForexTicker = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        setForexData(prev => prev.map(item => {
          ...
          if (baseCurrency === "USD") {
            ...
            // Convert non-USD pairs (e.g. EUR/USD = 1/USD/EUR)
    ...
    const simulateData = () => {
      setForexData(prev => prev.map(item => {
        const change = (Math.random() * 0.2 - 0.1); // Small random changes
        return {
          ...item,
    ...
    fetchForexData();
    const interval = setInterval(fetchForexData, 60000); // Update every 1 minute
